...
from .common import CorrelationMethod, kendalltau, rankdata
...
def _pearson_1xn(x: da.Array, data: da.Array) -> da.Array:
    # demeaned data
    x_dem = x - x.mean(axis=0)
    data_dem = data - data.mean(axis=0)

    num = (x_dem * data_dem).sum(axis=0)
    dom = x.shape[0] * x.std(axis=0) * data.std(axis=0)
...
def _spearman_1xn(x: da.Array, data: da.Array) -> da.Array:
    xrank = da.from_delayed(rankdata(x), dtype=np.float, shape=x.shape)
    ranks = da.from_delayed(rankdata(data), dtype=np.float, shape=data.shape)
...
def _kendall_tau_1xn(x: da.Array, data: da.Array) -> da.Array:
    ...
    for j in range(ncols):
        corr = da.from_delayed(kendalltau(x, data[:, j]), dtype=np.float, shape=())
        corrs.append(corr)

    # print(dask.compute(corrs)[0])
